...
# numerical stability clip for BCE so that I don't np.log(0)
_EPS = 1e-12


class MSELoss:
    """Mean squared error — used by the network to compute loss."""
...
class BCELoss:
    ...
    def gradient(self, y_true: np.ndarray, y_pred: np.ndarray):
        """Returns dL/dy_pred, after __call__ with the same inputs."""
        n = y_true.shape[0]
